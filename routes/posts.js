        ...
        if (req.user) {
            ...
            if (postIds.length > 0) {
                ...
                const [userFavorites] = await db.execute(
                    `SELECT post_id FROM user_favorites WHERE user_id = ? AND post_id IN (${postIds.map(() => '?').join(',')})`,
                    [req.user.id, ...postIds]
                );
...
router.post('/:id/favorite', authenticateToken, async (req, res) => {
    try {
        ...
        if (existingFavorite.length > 0) {
            // Quitar de favoritos
            await db.execute('DELETE FROM user_favorites WHERE user_id = ? AND post_id = ?', [userId, postId]);
            isFavorited = false;
        } else {
            // Agregar a favoritos
            await db.execute('INSERT INTO user_favorites (user_id, post_id) VALUES (?, ?)', [userId, postId]);
            isFavorited = true;
        }
